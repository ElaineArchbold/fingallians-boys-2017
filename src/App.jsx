...
function ContactForm({ player }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      formData.append("squad", "Fingallians 2017 Boys");
...
function RunLoggerV1({ week, runIndex, run, taskKey, done, canToggle, onToggle, showToast, player, ps, onGoProgress }) {
  const [open, setOpen] = useState(false);
  ...
  useEffect(() => { lastVoiceKmRef.current = lastVoiceKm; }, [lastVoiceKm]);
  ...
  function saveDraft(nextPoints = pointsRef.current, nextElapsed = elapsedRef.current) {
    try {
      localStorage.setItem(draftKey, JSON.stringify({
        ...
        points: nextPoints,
        elapsed: nextElapsed,
        distanceKm: Number(totalDistanceKm(nextPoints).toFixed(2)),
  ...
  function startGps() {
    ...
    const hasDraftPoints = pointsRef.current.length > 0;
    if (!hasDraftPoints) {
      setPoints([]);
      pointsRef.current = [];
      setElapsed(0);
    ...
    setPaused(false);
    pausedRef.current = false;
    setGpsStatus("Finding GPS signal… keep the app open where possible.");
    ...
    timerRef.current = setInterval(() => {
      if (!pausedRef.current) {
        setElapsed(v => {
          ...
          if (next % 5 === 0) saveDraft(pointsRef.current, next);
          return next;
        });
      }
    }, 1000);
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      pos => {
        ...
        if (accuracy > 60) {
          setGpsStatus(`Weak GPS signal (${Math.round(accuracy)}m). Still recording — move into open sky if possible.`);
          saveDraft(pointsRef.current, elapsedRef.current);
          return;
        }
        ...
        setPoints(prev => {
          ...
          if (last) {
            ...
            const seconds = Math.max(1, (next.ts - (last.ts || next.ts)) / 1000);
            const speedKmh = segmentKm / (seconds / 3600);

            if (segmentKm < 0.003) return prev;
            if (segmentKm > 0.35 && speedKmh > 28) {
              setGpsStatus("Ignored one jumpy GPS point. Still tracking from the last good point.");
              return prev;
            }
          }
          ...
          if (voiceKm >= 0.5 && voiceKm > lastVoiceKmRef.current) {
            lastVoiceKmRef.current = voiceKm;
            setLastVoiceKm(voiceKm);
            speak(`${voiceKm} kilometres completed`);
          }
          setGpsStatus(`GPS active · accuracy ${Math.round(accuracy)}m`);
          saveDraft(updated, elapsedRef.current);
          return updated;
        });
      },
      err => {
        console.error("Run GPS error", err);
        setGpsStatus("GPS signal dropped. Keep moving — the app will reconnect from your last good point.");
        showToast?.("GPS signal dropped briefly. Still recording from the last good point.");
        saveDraft(pointsRef.current, elapsedRef.current);
      },
  ...
  function finishGps() {
    ...
    setPaused(false);
    pausedRef.current = false;
    const distanceKm = Number(totalDistanceKm(finalPoints).toFixed(2));
    const durationMin = Math.max(1, Math.round(finalElapsed / 60));
    const saved = {
      type: "gps",
      week,
      taskKey,
  ...
  function beginFinishHold() {
    ...
    holdTimerRef.current = setInterval(() => {
      const pct = Math.min(100, ((Date.now() - holdStartRef.current) / 2200) * 100);
      setFinishHoldPct(pct);
      if (pct >= 100) {
        clearFinishHold();
        if (navigator.vibrate) {
    ...
    <>
      <div
        ...
        onClick={(e)=>{
          e.stopPropagation();
          if (!canToggle) return;
          if (done) setCompletedRunPromptOpen(true);
          else setOpen(true);
        }}
      >
      ...
      {completedRunPromptOpen && (
        <div className="run-modal-backdrop" onClick={() => setCompletedRunPromptOpen(false)}>
          <div className="run-modal" style={{maxWidth:420}} onClick={e=>e.stopPropagation()}>
            <button className="run-modal-close" onClick={() => setCompletedRunPromptOpen(false)}>×</button>
            ...
            <div style={{fontSize:13,color:"var(--muted)",lineHeight:1.45,margin:"6px 0 14px"}}>
              What would you like to do with this completed run?
            </div>
          ...
          <div className="run-modal" onClick={e=>e.stopPropagation()}>
            ...
            {mode === "gps" ? (
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:tracking?"1fr 1.2fr":"1fr",gap:8}}>
                  ...
                  <button
                    onMouseDown={beginFinishHold}
                    onMouseUp={cancelFinishHold}
                    onMouseLeave={cancelFinishHold}
                    onTouchStart={beginFinishHold}
                    onTouchEnd={cancelFinishHold}
                    onTouchCancel={cancelFinishHold}
                    onClick={(e)=>e.preventDefault()}
                    style={{border:0,background:tracking?`linear-gradient(90deg,#8b1a1a ${finishHoldPct}%,#c62828 ${finishHoldPct}%)`:"var(--g)",color:"#fff",borderRadius:12,padding:"11px 12px",fontWeight:900,fontFamily:"inherit",touchAction:"none"}}
                  >
...
function WeekDetail({ w, ps, pct, wPts, wMax, checks, onToggle, player, showToast, onGoProgress }) {
      ...
      {WEEKS.map((w,i) => {
        ...
        return (
          <div key={i} style={{marginBottom:24}}>
            <WeekDetail w={w} ps={ps} pct={pct} wPts={wP} wMax={wM} checks={checks} onToggle={onToggle} player={player} showToast={showToast} onGoProgress={onGoProgress} />
          </div>
        );
      })}
...
function WAMessageGenerator({ allPlayers, ptsMap }) {
  ...
  const message = [
    `🏑⚽ Fingallians 2017 Boys – Challenge Update · Week ${currentWeek} of 8`,
    ``,
    `Leaderboard so far:`,
...
function downloadCSV(allPlayers, ptsMap, weeklyMap) {
  ...
  const a    = document.createElement("a");
  a.href = url;
  a.download = `fingallians-2017-boys-challenge-${new Date().toISOString().slice(0,10)}.csv`;
  a.click();
  ...
  return (
    <div className="admin-wrap">
      <div style={{background:"linear-gradient(135deg,var(--g),#4a0a0e)",borderRadius:"var(--radius)",padding:"16px 18px",marginBottom:14,color:"#fff",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{fontSize:12,opacity:0.7,marginTop:2}}>Fingallians 2017 Boys · Week {currentWeek} of 8 · Summer Challenge 2026</div>
